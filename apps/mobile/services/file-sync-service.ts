...
async function downloadPmtiles(
  ...
  const localPath = getSheetPmtilesPath(organizationId, projectId, planId, sheetId)

  const dirPath = getSheetDirectoryById(organizationId, projectId, planId, sheetId)
...
export function useFileSyncService(projectId: string | undefined): FileSyncState {
  ...
  const [syncStatuses, setSyncStatuses] = React.useState<Map<string, SyncStatus>>(new Map())
  const [localPaths, setLocalPaths] = React.useState<Map<string, string>>(new Map())

  const sheets = store.useQuery(queryDb(tables.sheets.where(projectId ? { projectId } : {})))
...
export function useSheetPmtilesSync(
  ...
  React.useEffect(() => {
    ...
    const localFilePath = getSheetPmtilesPath(organizationId, projectId, planId, sheetId)
    ...
    downloadPmtiles(sheetId, projectId, planId, organizationId, remotePmtilesPath, sessionToken)
      .then((path) => {
        setLocalPath(path)
