...
import { type LayoutRegion, type ScheduleEntry, usePlanInfo } from "./use-plan-info"
...
function extractSnippet(content: string, query: string, radius = 30): string | null {
...
export function usePlanSearch(projectId: string, query: string): PlanSearchResult[] {
  const folders = useSheets(projectId)
  const { schedules, notes, scheduleEntriesByRegion, sheetNumberMap } = usePlanInfo(projectId)
  ...
  return useMemo(() => {
    ...
    for (const [regionId, entries] of scheduleEntriesByRegion) {
      ...
      for (const entry of entries) {
        ...
        if (entry.mark.toLowerCase().includes(trimmed)) {
          ...
          const propMatch = searchProperties(entry.properties, trimmed)
          if (propMatch) {
            matchText = propMatch
          }
        }
        ...
        if (matchText) {
          const typeLabel = entry.scheduleType.charAt(0).toUpperCase() + entry.scheduleType.slice(1)
          results.push({
    ...
    for (const region of notes) {
      ...
      if (region.regionTitle?.toLowerCase().includes(trimmed)) {
        ...
        const snippet = extractSnippet(region.extractedContent, trimmed)
        if (snippet) {
          matchText = snippet
        }
      }
