...
function inferMarkerType(label: string): CalloutMarker["type"] {
  ...
  if (upperLabel.includes("ELEV") || upperLabel.startsWith("E")) return "elevation"
  if (upperLabel.includes("SECT") || upperLabel.includes("/")) return "section"
  if (upperLabel.includes("NOTE")) return "note"
  return "detail"
}

function inferDiscipline(label: string): CalloutMarker["discipline"] | undefined {
  const upperLabel = label.toUpperCase()
  if (upperLabel.includes("A") || upperLabel.includes("ARCH")) return "arch"
  if (upperLabel.includes("S") || upperLabel.includes("STRUCT")) return "struct"
...
export function useMarkers(sheetId: string | null) {
  ...
  console.log(
    `[useMarkers] Raw query result for sheetId=${sheetId}:`,
    markers?.length ?? 0,
    "markers",
  )
