...
import { type TextProps as RNTextProps, View, type ViewProps } from "react-native"
...
function EmptyHeader({ className, children, ...props }: ViewProps) {
  return (
    <View className={cn("max-w-[320px] items-center gap-2", className)} {...props}>
      {children}
    </View>
  )
}
...
function EmptyTitle({ className, children, ...props }: EmptyTitleProps) {
  return (
    <Text
      className={cn("text-foreground text-center text-lg font-medium tracking-tight", className)}
      {...props}
    >
      {children}
    </Text>
...
function EmptyContent({ className, children, ...props }: ViewProps) {
  return (
    <View className={cn("w-full max-w-[320px] items-center gap-4", className)} {...props}>
      {children}
    </View>
  )
}
