...
import type { CalloutGroup as CalloutGroupType, TimelineSection } from "@/hooks/use-photos-timeline"
    ...
    <View className="py-4">
      <View className="mb-3 px-4">
        <View className="mb-2 flex-row items-center gap-2">
          ...
          <Text className="text-foreground text-sm font-semibold">{group.markerLabel}</Text>
          <Text className="text-muted-foreground text-xs">({group.photos.length} photos)</Text>
        </View>
        {hasIssues && (
          <Pressable
        ...
        <View className="mt-3 px-4">
          <View className="bg-muted/20 flex-row items-start gap-2 rounded-lg p-3">
            ...
            <View className="flex-1">
              <Text className="text-muted-foreground mb-1 text-xs">{mockVoiceNote.duration}</Text>
...
export const PhotoTimeline = React.memo(function PhotoTimeline({
  ...
  return (
    <SectionList
      ...
      renderItem={({ item }) => <CalloutGroup group={item} onPhotoPress={onPhotoPress} />}
