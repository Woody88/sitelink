...
import { ChevronDown, ChevronUp, ExternalLink, RefreshCcw, Sparkles, X } from "lucide-react-native"
...
export const DailySummaryBanner = React.memo(function DailySummaryBanner({
  ...
  return (
    <View
      className={cn("border-border bg-muted/20 overflow-hidden rounded-none border", className)}
    >
      {/* Title Bar */}
      <Pressable
          ...
          {isLoading ? (
            ...
            <View className="gap-3">
              {!hasSummary && buildDefaultSummary ? (
                <View className="gap-2">
                  {displaySummary.text.split("\n").map((line, index) => (
                    <Text key={index} className="text-foreground text-sm leading-relaxed">
                      {line}
                    </Text>
                  ))}
                </View>
              ...
              <View className="mt-1 flex-row items-center justify-between">
                {!hasSummary ? (
                  <Text className="text-muted-foreground text-xs">Default summary</Text>
                ) : (
                ...
                {hasSummary && (
                  ...
                  >
                    <Icon
                      as={RefreshCcw}
                      className={cn("text-muted-foreground size-3.5", isLoading && "animate-spin")}
                    />
              ...
              {!hasSummary && (
                <View className="mt-2 flex-row justify-end">
                  ...
                  >
                    <Icon
                      as={Sparkles}
                      className={cn("text-background size-4", isLoading && "animate-spin")}
                    />
        ...
        <View className="flex-1 items-center justify-center bg-black/50 p-4">
          <View className="bg-background border-border w-full max-w-sm rounded-xl border p-6">
            <View className="mb-4 flex-row items-center justify-between">
              <Text className="text-foreground text-lg font-bold">Generate AI Summary</Text>
              <Pressable onPress={() => setShowConfirmDialog(false)} className="p-1">
            ...
            <View className="flex-row gap-3">
              ...
              <Button onPress={handleConfirmGenerate} disabled={isLoading} className="flex-1">
