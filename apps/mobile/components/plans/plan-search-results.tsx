...
import { ChevronRight, FileText, StickyNote, Table2 } from "lucide-react-native"
...
import type { PlanSearchResult, SearchResultType } from "@/hooks/use-plan-search"
...
function SearchResultRow({ item, onPress }: { item: PlanSearchResult; onPress: () => void }) {
      ...
      <View className="flex-1">
        <View className="flex-row items-center gap-2">
          ...
          <View className="rounded-full px-2 py-0.5" style={{ backgroundColor: config.bg }}>
            <Text className="text-xs font-medium" style={{ color: config.labelColor }}>
              {config.label}
            </Text>
          </View>
        </View>
...
export function PlanSearchResults({
    ...
    switch (item.type) {
      ...
      case "schedule":
        if (item.region && item.entries)
          onSchedulePress(item.region, item.entries, item.sheetNumber)
        break
      case "notes":
        if (item.region) onNotesPress(item.region, item.sheetNumber)
        break
    }
  }
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => <SearchResultRow item={item} onPress={() => handlePress(item)} />}
    />
  )
}
