...
const STAGE_CONFIG: Record<ProcessingStage, StageConfig> = {
  waiting: { label: 'Waiting for connection', icon: Clock, color: 'text-blue-500' },
  images: { label: 'Generating images', icon: ImageIcon, color: 'text-blue-500' },
  metadata: { label: 'Extracting metadata', icon: FileText, color: 'text-purple-500' },
  callouts: { label: 'Detecting callouts', icon: AlertCircle, color: 'text-amber-500' },
  tiles: { label: 'Creating tiles', icon: Map, color: 'text-green-500' },
  review: { label: 'Review needed', icon: AlertCircle, color: 'text-orange-500' },
  completed: { label: 'Ready', icon: CheckCircle, color: 'text-green-500' },
}

const STAGE_ORDER: ProcessingStage[] = ['waiting', 'images', 'metadata', 'callouts', 'tiles']
...
function CircularProgress({ progress, size, strokeWidth, color = '#3b82f6' }: CircularProgressProps) {
...
export function PlanProcessingStatus({
  ...
  const isProcessing = stage !== 'waiting' && stage !== 'completed' && stage !== 'review'
  ...
  return (
    <AnimatedPressable
      ...
      <View className="flex-1">
        <Text className="text-foreground text-sm font-semibold">{config.label}</Text>
        {showProgress && (
          <Text className="text-muted-foreground text-xs">{Math.round(progress)}%</Text>
        )}
      </View>
      {stage === 'review' && calloutCount > 0 && (
        <Badge variant="secondary" className="bg-orange-500/10 border-orange-500/20">
          <Text className="text-xs font-bold text-orange-500">{calloutCount}</Text>
        </Badge>
      )}
    </AnimatedPressable>
  )
...
export function DetailedProcessingView({
  ...
  const getStageStatus = (stageType: ProcessingStage): 'completed' | 'active' | 'pending' => {
  ...
  return (
    <Modal animationType="slide" presentationStyle="pageSheet" visible transparent={false}>
      <View className="bg-background flex-1">
        ...
        <View className="flex-1 items-center justify-center px-6">
          <Animated.View style={animatedIconStyle} className="mb-8">
            <View className="relative items-center justify-center">
              {stage !== 'waiting' && stage !== 'completed' && stage !== 'review' && progress > 0 ? (
                <CircularProgress progress={progress} size={80} strokeWidth={4} />
              ) : (
                <Icon as={config.icon} className={cn('size-20', config.color)} />
              )}
            </View>
          </Animated.View>

          <Text className="text-foreground mb-2 text-2xl font-bold">{config.label}</Text>

          {stage !== 'waiting' && stage !== 'completed' && stage !== 'review' && progress > 0 && (
            <Text className="text-muted-foreground mb-8 text-lg">{Math.round(progress)}%</Text>
          )}
            ...
            <View className="mb-8 items-center gap-2">
              ...
              {onReviewCallouts && (
                <Pressable
                  ...
                  <Text className="text-sm font-bold text-white">Review Callouts</Text>
                </Pressable>
              )}
            </View>
          )}
            ...
            {STAGE_ORDER.map((stageType) => {
              ...
              return (
                <View
                  ...
                  {status === 'active' && stageType !== 'waiting' && progress > 0 && (
        ...
        <View
          ...
          <Text className="text-muted-foreground text-center text-xs leading-relaxed">
            This process may take a few minutes depending on the size of your plan
          </Text>
        </View>
      </View>
    </Modal>
