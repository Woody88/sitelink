  ...
  return (
    <View className="bg-background flex-1" style={{ paddingTop: insets.top }}>
      ...
      <View className="flex-row items-center gap-2 px-4 pb-3">
        <Text className="text-muted-foreground text-sm">Sheet {sheetNumber}</Text>
      ...
      {!region.extractedContent ? (
          ...
          {parsed.items.map((item) => (
            <View key={item.number} className="mb-4">
              <View className="flex-row">
                <Text className="text-foreground text-sm font-semibold w-8">{item.number}.</Text>
              ...
              {item.subItems?.map((sub) => (
                <View key={sub.letter} className="flex-row ml-8 mt-1.5">
                  <Text className="text-foreground text-sm w-6">{sub.letter}.</Text>
                  <Text className="text-foreground text-sm flex-1 leading-5">{sub.text}</Text>
                </View>
              ))}
            </View>
          ))}
        ...
        <ScrollView className="flex-1" contentContainerClassName="px-4 pb-8">
          <Text className="text-foreground text-sm leading-5">{region.extractedContent}</Text>
        </ScrollView>
      )}
