...
function SheetPill({ code, title, onPress }: SheetPillProps) {
  ...
  return (
    <AnimatedPressable
      ...
      style={animatedStyle}
      className="flex-1 flex-row items-center gap-3 bg-black/60 backdrop-blur-md rounded-xl px-4 py-3 active:bg-black/70"
    ...
    >
      ...
      {onPress && (
        <Icon as={ChevronDown} className="size-5 text-white/60" />
      )}
    </AnimatedPressable>
  )
}
...
function MarkersPill({ count, onPress }: MarkersPillProps) {
  ...
  return (
    <AnimatedPressable
      ...
      onPressOut={handlePressOut}
      style={animatedStyle}
      className="flex-row items-center gap-2 bg-primary/20 backdrop-blur-md rounded-xl px-3 py-3 active:bg-primary/30"
      accessibilityRole="button"
      accessibilityLabel={`${count} markers on this sheet. Tap to view markers.`}
    >
      <Icon as={MapPin} className="size-4 text-primary" />
...
export function CompactSheetInfo({ sheetCode, onPress, className }: CompactSheetInfoProps) {
  const scale = useSharedValue(1)
  ...
  return (
    <AnimatedPressable
      ...
      style={animatedStyle}
      className={cn(
        'flex-row items-center gap-2 bg-black/60 backdrop-blur-md rounded-full px-4 py-2 active:bg-black/70',
        className
      )}
    >
      <Icon as={Layers} className="size-4 text-white/80" />
      <Text className="text-white font-semibold text-sm">{sheetCode}</Text>
      {onPress && (
        <Icon as={ChevronDown} className="size-4 text-white/60" />
      )}
    </AnimatedPressable>
  )
}
