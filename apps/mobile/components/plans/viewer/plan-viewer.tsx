...
import { type CalloutMarker, usePlanViewer, type ViewerState } from "@/hooks/use-plan-viewer"
...
export function PlanViewer({
  ...
  console.log(
    `[PlanViewer] sheetId=${sheetId}, liveMarkers=${liveMarkers.length}, internalMarkers=${internalMarkers.length}, using=${markers.length}`,
  )
  ...
  const usePMTiles =
    processingStage === "tiles_generated" && (localPmtilesPath || remotePmtilesPath)
  ...
  const handleRegionPress = React.useCallback(async (region: LayoutRegionOverlay) => {
    await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)
    const regionLabel =
    ...
    <View className="bg-background flex-1">
      ...
      {isPmtilesDownloading && !isLoading && (
        ...
        >
          ...
          <Text className="mt-2 text-sm text-white/60">This happens once per sheet</Text>
        </Animated.View>
      )}
      ...
      {(error || pmtilesError) && (
        ...
        >
          ...
          <Text className="mb-6 text-center text-sm text-white/60">{pmtilesError || error}</Text>
          <Pressable
            onPress={handleRetry}
