...
export function PlanInfoView({ onRegionPress }: PlanInfoViewProps) {
  ...
  const { schedules, notes, legends, scheduleEntriesByRegion, sheetNumberMap } = usePlanInfo(
    projectId!,
  )
      ...
      <View className="flex-1 px-4 pt-8">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              Schedules, notes, and legends will appear here once plans are processed by
              SiteLink&apos;s AI
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
        ...
        {regions.map((region, index) => {
          ...
          return (
            <Pressable
              ...
              onPress={() =>
                onRegionPress?.(region, scheduleEntriesByRegion?.get(region.id), sheetNumber)
              }
            >
