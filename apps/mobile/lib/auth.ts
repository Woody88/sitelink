  ...
  if (result.data?.user) {
    try {
      const orgResult = await authClient.organization.create({
        ...
        slug: (data.organizationName || data.name).toLowerCase().replace(/\s+/g, "-"),
      })
      ...
      if (orgResult.data) {
        ...
        console.log("[Auth] Organization created and set as active:", orgResult.data.id)
      ...
      console.error("[Auth] Failed to create organization after signup:", error)
