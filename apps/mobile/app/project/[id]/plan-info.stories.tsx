...
const MOCK_SCHEDULE_ENTRIES_FOOTING: MockScheduleEntry[] = [
	{
		...
		confidence: 0.94,
		properties: { size: '24" x 12"', reinforcing: "4-#5 E.W.", concrete: "4000 PSI", notes: "Standard footing" },
	},
	{
		id: "e2",
		mark: "F2",
		scheduleType: "footing",
		confidence: 0.92,
		properties: { size: '36" x 18"', reinforcing: "5-#6 E.W.", concrete: "4000 PSI", notes: "Heavy load footing" },
	},
	{
		id: "e3",
		mark: "F3",
		scheduleType: "footing",
		confidence: 0.88,
		properties: { size: '30" x 15"', reinforcing: "4-#5 E.W.", concrete: "5000 PSI", notes: "Corner footing" },
	},
];
...
const MOCK_SCHEDULE_ENTRIES_SLAB: MockScheduleEntry[] = [
	{
		...
		scheduleType: "slab",
		confidence: 0.96,
		properties: { thickness: '6"', concrete: "4000 PSI", reinforcing: "#4@12\" O.C. E.W.", finish: "Broom" },
	},
	{
		id: "e5",
		mark: "SL2",
		scheduleType: "slab",
		confidence: 0.93,
		properties: { thickness: '8"', concrete: "4000 PSI", reinforcing: "#5@10\" O.C. E.W.", finish: "Trowel" },
	},
	{
		id: "e6",
		mark: "SL3",
	...
	},
	{
		...
		scheduleType: "slab",
		confidence: 0.89,
		properties: { thickness: '10"', concrete: "5000 PSI", reinforcing: "#5@8\" O.C. E.W.", finish: "Trowel" },
	},
];
	...
	},
	{
		...
		notesContent: JSON.stringify({
			...
			items: [
				...
				{ number: 6, text: "Contractor shall verify all dimensions and conditions in field prior to commencing work." },
			],
		}),
	},
	{
		...
		notesContent: JSON.stringify({
			...
			items: [
				...
				{ number: 3, text: "Concrete shall not be placed when ambient temperature is below 40\u00b0F." },
			],
		}),
	},
	{
						...
						<View className="flex-1">
							...
							{region.entries && (
								<Text className="text-muted-foreground text-xs">
									...
									{region.entries.length === 1 ? "entry" : "entries"}
								</Text>
							)}
						</View>
...
function ScheduleRowDetail({
	...
	return (
		...
		>
			...
			<Pressable
				className="bg-primary mt-4 items-center rounded-xl py-3"
			>
				<View className="flex-row items-center gap-2">
...
function ScheduleDetailView({
	...
	const columns =
		entries.length > 0
			? ["Mark", ...Object.keys(entries[0].properties)]
			: [];
	...
	return (
		<View className="bg-background flex-1" style={{ minHeight: "100vh", position: "relative" } as any}>
					...
					<View className="min-w-full">
						...
						{entries.map((entry, rowIndex) => (
							...
							>
								{columns.map((col) => {
									const value =
										col === "Mark" ? entry.mark : (entry.properties[col] ?? "");
									return (
...
function NotesDetailView({
	...
	let parsed: { title: string; items: { number: number; text: string; subItems?: { letter: string; text: string }[] }[] } | null = null;
	try {
						...
						{parsed.items.map((item) => (
							<View key={item.number}>
								<View className="flex-row gap-3">
									<Text className="text-muted-foreground text-sm font-bold" style={{ minWidth: 24 }}>
										{item.number}.
									</Text>
