...
import { MembersScreen, ProjectSettingsScreen } from "@/app/_story-components"

function ProjectSettingsStory() {
  const [screen, setScreen] = React.useState<"settings" | "members">("settings")

  if (screen === "members") {
    return <MembersScreen onBack={() => setScreen("settings")} />
  }

  return <ProjectSettingsScreen onNavigateToMembers={() => setScreen("members")} />
}
