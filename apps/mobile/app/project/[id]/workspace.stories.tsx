	...
	return (
		<Empty className="mx-4 mb-4">
			<EmptyHeader>
				...
				<EmptyDescription>
					Photos and recordings from this project will appear here as they are
					captured.
				</EmptyDescription>
			</EmptyHeader>
		</Empty>
				...
				<View className="px-4 pb-4">
					...
					{state === "loading" ? (
						...
						<View className="gap-3">
							<Text className="text-foreground text-sm leading-relaxed">
								Good progress today on the Holabird Ave Warehouse project. Mike
								flagged a junction box clearance issue at 5/A7 that needs
								resolution before the conduit run can proceed. Sarah documented
								panel rough-in progress at 3/A2 with 3 photos showing the
								installation is tracking to schedule. One voice note captured
								with details on the junction box repositioning needed.
							</Text>
				...
				<Pressable
					...
					onPress={() => {
						setSelectedMarkerId((prev) =>
							prev === marker.id ? undefined : marker.id,
						);
								...
								<Collapsible key={group.id} defaultOpen>
									...
									<CollapsibleContent>
										{group.entries.map((entry, index) => {
											const summary = Object.values(entry.properties)
												.slice(0, 2)
												.join("  ");
											return (
												<View key={entry.id}>
				...
				>
					...
					{metadata.hasVoiceNote && metadata.voiceNote && (
						<View className="flex-row items-start gap-2 rounded-lg bg-white/5 p-3">
							...
							<View className="flex-1">
								<Text className="mb-1 text-xs text-white/50">
									{metadata.voiceNote.duration}
								</Text>
...
function ProjectWorkspace({
	...
	if (screen.type === "members") {
		return (
			<MembersScreen
				onBack={() => setScreen({ type: "project-settings" })}
			/>
		);
	}
	...
	if (screen.type === "camera") {
		return (
			<InlineCamera onClose={() => setScreen({ type: "workspace" })} />
		);
	}
	...
	return (
		...
		>
			...
			{activeView === "plans" && (
				<PlansTab
					onSheetPress={(sheet) =>
						setScreen({ type: "plan-viewer", sheet })
					}
				/>
			)}
			{activeView === "media" && (
