...
function MarkerDetailSheet({
  ...
  return (
    ...
    >
      ...
      <Text className="text-muted-foreground mb-5 text-sm">{marker.description}</Text>
...
function StoryScheduleRow({ entry, index }: { entry: MockScheduleEntry; index: number }) {
...
function PlanViewerScreen({
  ...
  return (
    <View style={{ minHeight: "100vh", position: "relative", backgroundColor: "#0a0a0a" } as any}>
      ...
      {/* Schedule drawer */}
      {showScheduleDrawer && <ScheduleDrawerPanel onClose={() => setShowScheduleDrawer(false)} />}
    </View>
  )
}
