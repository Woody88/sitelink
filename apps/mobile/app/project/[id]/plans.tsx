...
import { type LayoutRegion, type ScheduleEntry, usePlanInfo } from "@/hooks/use-plan-info"
...
export default function PlansScreen() {
  if (isPrototypeMode()) return <PrototypePlansScreen />
  return <LivePlansScreen />
}

function PrototypePlansScreen() {
  ...
  React.useEffect(() => {
    if (MOCK_FOLDERS.length > 0 && !hasInitialized.current) {
      setExpandedFolders([MOCK_FOLDERS[0].id])
  ...
  const filteredFolders = MOCK_FOLDERS.map((folder) => ({
    ...
    (folder) =>
      folder.name.toLowerCase().includes(searchQuery.toLowerCase()) || folder.sheets.length > 0,
  )
...
function LivePlansScreen() {
  ...
  const handleSheetChange = React.useCallback(
    (sheetRef: string) => {
      const allSheets = folders.flatMap((f) => f.sheets)
      const sheet = allSheets.find((s) => s.id === sheetRef || s.number === sheetRef)
      if (sheet) {
    ...
    <View className="bg-background flex-1">
      ...
      {isSearchActive ? (
        ...
        <ScrollView className="flex-1" contentContainerClassName="px-4 pb-8">
          ...
          {(() => {
            const totalItems =
              planInfo.schedules.length + planInfo.notes.length + planInfo.legends.length
                    ...
                    {allRegions.map((region) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Text className="text-muted-foreground mr-2 text-sm">{sheetNumber}</Text>
          ...
          {!isLoading && folders.length === 0 && (
            <View className="flex-1 items-center justify-center py-20">
              ...
              <Text className="text-foreground mb-2 text-lg font-semibold">No Plans Yet</Text>
      ...
      >
        {selectedPlan && selectedSheet && (
          <PlanViewer
            ...
            imageUrl={selectedSheet.imagePath || "https://picsum.photos/2000/1500"}
