...
export default function ProjectWorkspaceLayout() {
  if (isPrototypeMode()) return <PrototypeWorkspaceLayout />
  return <LiveWorkspaceLayout />
}

function PrototypeWorkspaceLayout() {
  ...
  const handleFABPress = useCallback(() => {
    if (activeView === "plans") {
      Alert.alert("Prototype Mode", "Plan upload is not available in prototype mode.")
    } else {
...
function LiveWorkspaceLayout() {
  ...
  const projectQuery = useMemo(() => queryDb(tables.projects.where({ id: params.id })), [params.id])
  ...
  const handleUploadFromDevice = useCallback(async () => {
    try {
      ...
      Alert.alert("Error", "Failed to upload and process plan. Please try again.", [{ text: "OK" }])
    }
  }, [pickAndUploadPlan])
  ...
  return (
    <>
      ...
      <View className="bg-background flex-1">
        ...
        {/* Upload Modal */}
        <Modal visible={uploadProgress?.status === "uploading"} transparent animationType="fade">
          <View className="flex-1 items-center justify-center bg-black/50">
            <View className="bg-background mx-8 items-center rounded-xl p-6">
              <ActivityIndicator size="large" />
              <Text className="text-foreground mt-4 text-lg font-semibold">Uploading Plan</Text>
