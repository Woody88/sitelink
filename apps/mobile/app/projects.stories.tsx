          ...
          {state === "loading" ? (
            ...
            <View className="gap-3">
              <Text className="text-foreground text-sm leading-relaxed">
                Good progress today on the project. The team captured 5 photos documenting current
                conditions, flagged 1 issue requiring attention, and recorded a voice note with
    ...
    <View className="bg-background" style={{ minHeight: "100vh", position: "relative" } as any}>
      ...
      <View className="bg-background" style={{ paddingTop: 8 }}>
        <View className="min-h-[56px] flex-row items-center justify-between px-4">
          ...
          <Pressable
            onPress={() => onNavigate({ type: "project-settings", project })}
      ...
      {activeTab === 0 && (
        <>
          ...
          <PlansTab onSheetPress={(sheet) => onNavigate({ type: "plan-viewer", project, sheet })} />
        </>
      )}
      {activeTab === 1 && (
...
function ProjectsScreen({
  ...
  const [projectsList, setProjectsList] = React.useState(() => (isEmpty ? [] : projects))
  ...
  if (screen.type === "notifications") {
    return <NotificationsScreen onBack={() => setScreen({ type: "projects" })} />
  }
  ...
  if (
    ...
    screen.type === "project-settings" ||
    screen.type === "members" ||
    screen.type === "plan-viewer" ||
    screen.type === "camera"
  ) {
    const project =
      screen.type === "workspace"
    ...
    if (screen.type === "project-settings") {
      return (
        <ProjectSettingsScreen
          ...
          onNavigateToMembers={() => setScreen({ type: "members", project })}
        />
      )
    }

    if (screen.type === "members") {
      return <MembersScreen onBack={() => setScreen({ type: "project-settings", project })} />
    }
    ...
    if (screen.type === "camera") {
      return <InlineCamera onClose={() => setScreen({ type: "workspace", project })} />
    }
    ...
    <View className="bg-background" style={{ minHeight: "100vh", position: "relative" } as any}>
      ...
      <View className="border-border flex-row items-center justify-between border-b px-4 py-3">
        ...
        <Pressable
          ...
          style={{ width: 44, height: 44 }}
        >
          <Icon as={darkMode ? Sun : Moon} className="text-foreground size-6" />
        </Pressable>
      </View>
      ...
      {filteredProjects.length > 0 ? (
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <ProjectListItem
              ...
              onPress={() => setScreen({ type: "workspace", project: item })}
            />
          )}
        ...
        <Empty className="mx-4 mb-4">
          ...
          <EmptyContent>
            <Button className="h-12 rounded-xl px-8" onPress={() => setModal("create-project")}>
