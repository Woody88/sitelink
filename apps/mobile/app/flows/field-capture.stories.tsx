...
const MOCK_LOCATIONS: PlanLocation[] = [
	...
	{ id: "loc2", label: "3/A2", sheetNumber: "S1.0", sheetTitle: "Foundation Plan", type: "section" },
	{ id: "loc3", label: "2/A1", sheetNumber: "S2.0", sheetTitle: "Structural Details", type: "detail" },
	{ id: "loc4", label: "F1", sheetNumber: "S1.0", sheetTitle: "Foundation Plan", type: "area" },
	{ id: "loc5", label: "E1", sheetNumber: "S3.0", sheetTitle: "Sections & Elevations", type: "elevation" },
];
...
function VoiceRecordingOverlay({ duration, onStop, onCancel }: { duration: string; onStop: () => void; onCancel: () => void }) {
	return (
		<View style={{ position: "absolute", bottom: 120, left: 16, right: 16, zIndex: 25 }}>
			<View className="flex-row items-center gap-4 rounded-2xl px-5 py-4" style={{ backgroundColor: "rgba(28,28,28,0.95)" }}>
				...
				<View className="flex-1 flex-row items-center justify-center gap-0.5">
					{[0.3, 0.7, 0.5, 0.9, 0.4, 0.8, 0.6, 1.0, 0.5, 0.7, 0.3, 0.6, 0.8, 0.4, 0.9, 0.5, 0.7, 0.3].map((h, i) => (
						<View key={i} className="rounded-full bg-red-400/60" style={{ width: 3, height: Math.max(4, h * 24) }} />
					))}
				</View>
				<Text className="text-sm font-semibold text-white" style={{ fontVariant: ["tabular-nums"] }}>{duration}</Text>
				<Pressable onPress={onStop} className="items-center justify-center rounded-full bg-red-500" style={{ width: 36, height: 36 }}>
...
function LinkToPlanSheet({ onClose, onSelect, autoSuggestion }: { onClose: () => void; onSelect: (location: PlanLocation) => void; autoSuggestion?: PlanLocation }) {
	return (
		<View style={{ position: "absolute", top: 0, bottom: 0, left: 0, right: 0, zIndex: 40 }}>
			...
			<View style={{ position: "absolute", bottom: 0, left: 0, right: 0, maxHeight: "75%", backgroundColor: "#1c1c1c", borderTopLeftRadius: 24, borderTopRightRadius: 24 }}>
				...
				<View className="flex-row items-center justify-between px-6 pb-3">
					<View>
						...
						<Text className="text-muted-foreground text-sm">Associate this photo with a callout location</Text>
					</View>
				...
				{autoSuggestion && (
					<View className="px-6 pb-3">
						<View className="flex-row items-center gap-3 rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(34,197,94,0.08)", borderWidth: 1, borderColor: "rgba(34,197,94,0.2)" }}>
							...
							<View className="flex-1">
								<Text className="text-xs font-medium" style={{ color: "#22c55e" }}>Auto-detected from current view</Text>
								<Text className="text-foreground text-sm font-semibold">{autoSuggestion.label} on {autoSuggestion.sheetNumber}</Text>
							</View>
							<Pressable onPress={() => onSelect(autoSuggestion)} className="rounded-lg px-3 py-2" style={{ backgroundColor: "rgba(34,197,94,0.15)" }}>
								<Text style={{ color: "#22c55e" }} className="text-xs font-bold">Use</Text>
							</Pressable>
						</View>
					</View>
				)}
				...
				<View className="px-6 pb-3">
					<View className="bg-muted/30 flex-row items-center rounded-xl px-3" style={{ height: 40 }}>
						<Icon as={Search} className="text-muted-foreground mr-2 size-4" />
						<Text className="text-muted-foreground flex-1 text-sm">Search callouts or sheets...</Text>
					</View>
				</View>

				<ScrollView className="flex-1" showsVerticalScrollIndicator>
					<Text className="text-muted-foreground px-6 pb-2 text-xs font-bold uppercase tracking-wider">All Callouts</Text>
					{MOCK_LOCATIONS.map((location) => {
						const color = TYPE_COLORS[location.type];
						return (
							<Pressable key={location.id} onPress={() => onSelect(location)} className="active:bg-muted/20 flex-row items-center gap-3 px-6 py-3">
								<View className="items-center justify-center rounded-lg" style={{ width: 36, height: 36, backgroundColor: color + "15" }}>
								...
								<View className="flex-1">
									...
									<Text className="text-muted-foreground text-xs">{location.sheetNumber} — {location.sheetTitle}</Text>
								</View>
								<View className="rounded-full px-2 py-0.5" style={{ backgroundColor: color + "15" }}>
									<Text style={{ color, fontSize: 10, fontWeight: "600" }}>{location.type.charAt(0).toUpperCase() + location.type.slice(1)}</Text>
								</View>
...
type CapturePhase = "viewfinder" | "preview" | "linking" | "recording" | "rfi-prompt" | "voice-error";
...
function FieldCaptureFlow({
	...
	const [linkedLocation, setLinkedLocation] = React.useState<PlanLocation | undefined>(initialLinked ? MOCK_LOCATIONS[0] : undefined);
	...
	if (phase === "linking") {
		return (
			<View style={{ minHeight: "100vh", position: "relative", backgroundColor: "#000" } as any}>
				<Image source={{ uri: "https://picsum.photos/seed/construct99/1080/1920" }} style={{ width: "100%", height: "100%", position: "absolute", opacity: 0.3 }} resizeMode="cover" />
				<LinkToPlanSheet
					onClose={() => setPhase("preview")}
					onSelect={(loc) => { setLinkedLocation(loc); setPhase("preview"); setToastMsg(`Linked to ${loc.label} on ${loc.sheetNumber}`); }}
				...
				<View style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 20, backgroundColor: "rgba(0,0,0,0.78)", borderTopLeftRadius: 24, borderTopRightRadius: 24, borderTopWidth: 1, borderColor: "rgba(255,255,255,0.1)" }}>
					...
					<View style={{ paddingHorizontal: 20, paddingBottom: 40 }}>
						<View className="items-center gap-4 py-4">
							<View className="items-center justify-center rounded-full" style={{ width: 64, height: 64, backgroundColor: "rgba(239,68,68,0.12)" }}>
								<Icon as={AlertCircle} style={{ color: "#ef4444" }} className="size-8" />
							</View>
							<View className="items-center gap-1.5">
								<Text style={{ color: "#ebebeb", fontSize: 16, fontWeight: "700" }}>Transcription Failed</Text>
	...
	if (phase === "recording") {
		return (
			<View style={{ minHeight: "100vh", position: "relative", backgroundColor: "#000" } as any}>
				<Image source={{ uri: "https://picsum.photos/seed/construct99/1080/1920" }} style={{ width: "100%", height: "100%", position: "absolute" }} resizeMode="cover" />
				{/* Dim overlay so recording UI pops */}
				<View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.3)", zIndex: 5 }} />
				{/* Floating saved indicator */}
				<View style={{ position: "absolute", top: 16, left: 12, zIndex: 20, flexDirection: "row", alignItems: "center", gap: 8 }}>
					<View className="flex-row items-center gap-2 rounded-full px-3 py-2" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
						<View className="items-center justify-center rounded-full bg-green-500/20" style={{ width: 24, height: 24 }}>
				...
				<VoiceRecordingOverlay
					...
					onStop={() => { setHasVoiceNote(true); setPhase("preview"); }}
	...
	if (phase === "rfi-prompt") {
		return (
			<View style={{ minHeight: "100vh", position: "relative", backgroundColor: "#000" } as any}>
				<Image source={{ uri: "https://picsum.photos/seed/construct99/1080/1920" }} style={{ width: "100%", height: "100%", position: "absolute", opacity: 0.3 }} resizeMode="cover" />
				<View style={{ position: "absolute", top: 0, bottom: 0, left: 0, right: 0, zIndex: 40 }}>
					<Pressable onPress={() => setPhase("preview")} style={{ position: "absolute", top: 0, bottom: 0, left: 0, right: 0, backgroundColor: "rgba(0,0,0,0.5)" }} />
					<View style={{ position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#1c1c1c", borderTopLeftRadius: 24, borderTopRightRadius: 24 }}>
						...
						<View className="px-6 pb-6">
							...
							<View className="gap-3">
								<Pressable onPress={() => setToastMsg("Opening RFI generator...")} className="bg-primary flex-row items-center justify-center gap-2 rounded-xl py-3.5">
								...
								<Pressable onPress={() => setPhase("preview")} className="flex-row items-center justify-center rounded-xl py-3.5" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
		...
		return (
			<View style={{ minHeight: "100vh", position: "relative", backgroundColor: "#000" } as any}>
				...
				<View style={{ position: "absolute", top: 16, left: 12, zIndex: 20, flexDirection: "row", alignItems: "center", gap: 10 }}>
					<Pressable onPress={() => setPhase("viewfinder")} className="items-center justify-center rounded-full" style={{ width: 40, height: 40, backgroundColor: "rgba(0,0,0,0.5)" }}>
				...
				{showOCR && (
					<Pressable
						onPress={() => { setCopied(true); setToastMsg("Copied to clipboard"); setTimeout(() => setCopied(false), 2000); }}
						style={{ position: "absolute", top: 16, right: 12, zIndex: 20 }}
					>
						<View className="flex-row items-center gap-2 rounded-2xl px-4 py-2.5" style={{ backgroundColor: "rgba(0,0,0,0.75)", maxWidth: 260 }}>
							<Icon as={ScanText} className="size-4" style={{ color: "#3b82f6" }} />
							<View className="flex-1">
								<Text style={{ color: "rgba(255,255,255,0.5)", fontSize: 10, fontWeight: "600" }}>Detected text</Text>
								<Text style={{ color: "#ebebeb", fontSize: 12, fontWeight: "600" }} numberOfLines={1}>SIEMENS Model: 5SY4-106-7</Text>
							</View>
							<Icon as={copied ? Check : Copy} className="size-3.5" style={{ color: copied ? "#22c55e" : "rgba(255,255,255,0.5)" }} />
						</View>
					</Pressable>
				)}
					...
					<View style={{ paddingHorizontal: 20, paddingBottom: 40 }}>
						...
						<View className="flex-row items-center gap-3 mb-4">
							...
							<View className="flex-1">
								{linkedLocation ? (
									...
									<Text style={{ color: "#ebebeb", fontSize: 14, fontWeight: "600" }}>Photo captured</Text>
								)}
							</View>
						</View>
						...
						{hasVoiceNote ? (
							<View style={{ backgroundColor: "rgba(255,255,255,0.1)", borderRadius: 12, padding: 14, marginBottom: 16 }}>
								...
								<Pressable onPress={() => setPhase("recording")} className="mt-3 self-start">
									<Text style={{ color: "#3b82f6", fontSize: 12, fontWeight: "600" }}>Re-record</Text>
								</Pressable>
							</View>
						) : (
							<Pressable
						...
						{!linkedLocation && (
							<View className="flex-row items-center gap-2 mb-4">
								<Icon as={MapPin} className="size-3.5" style={{ color: "rgba(255,255,255,0.35)" }} />
						...
						{/* Action buttons */}
						<View className="flex-row items-center gap-3">
							<Pressable onPress={() => setPhase("viewfinder")} className="flex-row items-center justify-center gap-2 rounded-xl py-3.5" style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.1)" }}>
	...
	return (
		<View style={{ minHeight: "100vh", position: "relative", backgroundColor: "#000" } as any}>
			...
			<View style={{ position: "absolute", top: 580, left: 0, right: 0, zIndex: 15, alignItems: "center" }}>
				{linkedLocation ? (
					<Pressable onPress={() => setPhase("linking")} className="flex-row items-center" style={{ backgroundColor: "rgba(0,0,0,0.7)", borderRadius: 20, height: 36, paddingHorizontal: 14, gap: 6 }}>
						<Icon as={MapPin} style={{ color: "#ffffff" }} className="size-3.5" />
						<Text style={{ color: "#eab308", fontSize: 13, fontWeight: "700" }}>{linkedLocation.label}</Text>
					...
					<View className="flex-row items-center" style={{ backgroundColor: "rgba(0,0,0,0.7)", borderRadius: 20, height: 36, paddingHorizontal: 14, gap: 6 }}>
						...
						<Pressable onPress={() => setPhase("linking")} className="bg-primary ml-2 rounded-full px-3 py-1.5">
