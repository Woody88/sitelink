...
import { ArrowLeft, ChevronRight, Plus, Search, UserPlus, Users, X } from "lucide-react-native"
import * as React from "react"
import { Pressable, ScrollView, View } from "react-native"
import { FlatRow, StoryHeader, StoryToast } from "@/app/_story-components"
...
function TeamManagementFlow({
  initialScreen = "settings" as FlowScreen,
}: {
  ...
  if (screen === "settings") {
    return (
      <View className="bg-background" style={{ minHeight: "100vh" } as any}>
        ...
        >
          <View className="pt-6">
            <Text className="text-muted-foreground mb-3 px-4 text-sm font-semibold tracking-wider uppercase">
              Team
            </Text>
            <FlatRow
              icon={Users}
              label="Team Members"
              sublabel={`${STORY_MEMBERS.length} members`}
              onPress={() => setScreen("members")}
              right={<Icon as={ChevronRight} className="text-muted-foreground size-5" />}
            />
          </View>
        </ScrollView>
      </View>
    ...
    <View className="bg-background" style={{ minHeight: "100vh", position: "relative" } as any}>
      ...
      <ScrollView className="flex-1" contentContainerClassName="py-2">
        {filteredMembers.map((member, idx) => (
          <View
            key={member.id}
            className={cn(idx < filteredMembers.length - 1 && "border-border/30 border-b")}
          >
        ...
        {pendingMembers.map((pm, i) => (
          <View key={`pending-${i}`} className={cn("border-border/30 border-b opacity-60")}>
            <View className="flex-row items-center justify-between px-4 py-3">
              <View className="flex-row items-center gap-3">
              ...
              <Text className="text-muted-foreground text-sm">{pm.role}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
...
export const ProjectSettings: Story = {
  name: "1. Project Settings",
  args: { initialScreen: "settings" },
}
