...
import { Camera, Info, Plus, Shield, UserPlus } from "lucide-react-native"
  ...
  return (
    <View className="gap-2">
      <Pressable
        ...
        className="flex-row items-center gap-3 px-4 py-4 opacity-40"
      >
      ...
      {showTooltip && (
        <View
          className="ml-4 rounded-lg px-4 py-2.5"
    ...
    <View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
      ...
      <View className="px-4 pt-6 gap-6">
        <View
          ...
          style={{ backgroundColor: "rgba(59,130,246,0.08)", borderRadius: 8 }}
        >
        ...
        <View className="gap-1">
          <Text className="text-muted-foreground px-4 text-xs font-bold tracking-wider uppercase">
            Actions
          </Text>
          <DisabledActionRow
            icon={Camera}
  ...
  return (
    <View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
      ...
      <View className="px-4 pt-6 gap-6">
        ...
        <View className="gap-2">
          {members.map((m) => (
            <View key={m.initials} className="flex-row items-center gap-3 px-4 py-3">
              <View
  ...
  return (
    <View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
      ...
      <View className="px-4 pt-6 gap-6">
        <View
          className="flex-row items-center gap-2 px-4 py-2.5"
          style={{ backgroundColor: "rgba(34,197,94,0.08)", borderRadius: 8 }}
        >
...
function PermissionStatesFlow({
  screen = "viewer-no-photos" as ScreenType,
}: {
  screen?: ScreenType
}) {
  switch (screen) {
...
export const ViewerNoPhotos: Story = {
  name: "1. Viewer - No Photos",
  args: { screen: "viewer-no-photos" },
}
export const ViewerNoInvite: Story = {
  name: "2. Viewer - No Invite",
  args: { screen: "viewer-no-invite" },
}
export const AdminFullAccess: Story = {
  name: "3. Admin - Full Access",
  args: { screen: "admin-full" },
}
