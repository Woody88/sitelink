	...
	return (
		<View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
			...
			<ScrollView contentContainerClassName="px-4 pb-8">
				<View className="mt-2 overflow-hidden rounded-2xl" style={{ backgroundColor: "rgba(255,255,255,0.06)" }}>
					...
					<View className="flex-row px-5 pb-5">
						<View className="flex-1 items-center rounded-xl py-3" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
				...
				<View className="mt-4 rounded-2xl px-5 py-4" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
					<Text className="text-muted-foreground mb-3 text-xs font-bold uppercase tracking-wider">Recent Activity</Text>
					{[
...
function ShareOptionsSheet({ onClose, onCopy, onSend }: { onClose: () => void; onCopy: () => void; onSend: (method: string) => void }) {
		...
		<View style={{ minHeight: "100vh", position: "relative" } as any}>
			...
			<View style={{ position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#1c1c1c", borderTopLeftRadius: 24, borderTopRightRadius: 24 }}>
				...
				<View className="px-6 pt-4">
					<View className="flex-row items-center rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(59,130,246,0.08)", borderWidth: 1, borderColor: "rgba(59,130,246,0.2)" }}>
						<Icon as={Link} className="size-4 mr-3" style={{ color: "#3b82f6" }} />
						<Text className="flex-1 text-sm font-medium" style={{ color: "#93c5fd" }} numberOfLines={1}>{SHARE_LINK}</Text>
						<Pressable onPress={onCopy} className="ml-2 rounded-lg px-3 py-1.5" style={{ backgroundColor: "rgba(59,130,246,0.15)" }}>
							<Text style={{ color: "#3b82f6" }} className="text-xs font-bold">Copy</Text>
						</Pressable>
					</View>
				</View>
				...
				<View className="px-6 pt-5">
					<Text className="text-muted-foreground mb-3 text-xs font-bold uppercase tracking-wider">Link Settings</Text>
					...
					<View className="overflow-hidden rounded-xl" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
						...
						>
							<View className="flex-row items-center gap-3">
								<Icon as={requireSignIn ? Lock : Shield} className="size-4" style={{ color: requireSignIn ? "#f59e0b" : "#6b7280" }} />
								<View>
									<Text className="text-foreground text-sm font-medium">Require sign-in</Text>
									<Text className="text-muted-foreground text-xs">Recipients must create an account</Text>
								</View>
							</View>
							<View
						...
						>
							...
							<View className="flex-row items-center gap-1.5">
								<Text className="text-sm font-semibold" style={{ color: "#3b82f6" }}>{EXPIRY_OPTIONS[expiryIndex]}</Text>
						...
						{showExpiryDropdown && (
							<View className="px-4 pb-3">
								{EXPIRY_OPTIONS.map((option, idx) => (
									<Pressable
										key={option}
										onPress={() => { setExpiryIndex(idx); setShowExpiryDropdown(false); }}
										className="flex-row items-center justify-between rounded-lg px-3 py-2.5"
										style={{ backgroundColor: idx === expiryIndex ? "rgba(59,130,246,0.1)" : "transparent" }}
									>
										<Text className="text-sm" style={{ color: idx === expiryIndex ? "#3b82f6" : "#9ca3af" }}>{option}</Text>
										{idx === expiryIndex && <Icon as={Check} className="size-4" style={{ color: "#3b82f6" }} />}
									</Pressable>
								))}
							</View>
						)}
	...
	return (
		<View className="bg-background flex-1 items-center justify-center" style={{ minHeight: "100vh" } as any}>
			<View className="items-center gap-5 px-8">
				...
				<View className="w-full flex-row items-center rounded-xl px-4 py-3.5 mt-2" style={{ backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}>
					<Icon as={Link} className="size-4 mr-3" style={{ color: "#3b82f6" }} />
					<Text className="text-foreground flex-1 text-sm font-medium" numberOfLines={1}>{SHARE_LINK}</Text>
					<Pressable className="ml-2">
						<Icon as={ClipboardCopy} className="size-4" style={{ color: "#6b7280" }} />
					</Pressable>
				</View>

				<View className="w-full mt-2 rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(59,130,246,0.06)", borderWidth: 1, borderColor: "rgba(59,130,246,0.12)" }}>
					<View className="flex-row items-center gap-2">
						<Icon as={Eye} className="size-3.5" style={{ color: "#3b82f6" }} />
						<Text className="text-xs font-medium" style={{ color: "#93c5fd" }}>View-only access</Text>
					</View>
...
function RecipientViewScreen() {
	return (
		<View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
			<View className="flex-row items-center justify-center px-4 py-2.5" style={{ backgroundColor: "rgba(59,130,246,0.1)", borderBottomWidth: 1, borderColor: "rgba(59,130,246,0.15)" }}>
				<Icon as={Eye} className="size-3.5 mr-2" style={{ color: "#3b82f6" }} />
				<Text className="text-xs font-semibold" style={{ color: "#93c5fd" }}>
					View-only
				</Text>
				<Text className="text-xs mx-1.5" style={{ color: "rgba(147,197,253,0.4)" }}>{"·"}</Text>
			...
			<ScrollView contentContainerClassName="px-4 pb-32">
				<View className="mt-2 rounded-2xl overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
					<View className="px-5 pt-4 pb-3">
						<Text className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Plan Sheets</Text>
					</View>
					{[
						...
						<View key={sheet.number} className="flex-row items-center gap-3 px-5 py-3" style={{ borderTopWidth: 1, borderColor: "rgba(255,255,255,0.04)" }}>
							<View className="items-center justify-center rounded-lg" style={{ width: 40, height: 40, backgroundColor: "rgba(59,130,246,0.1)" }}>
								<Text className="text-xs font-bold" style={{ color: "#3b82f6" }}>{sheet.number}</Text>
							</View>
							<View className="flex-1">
								<Text className="text-foreground text-sm font-semibold">{sheet.title}</Text>
								<Text className="text-muted-foreground text-xs">{sheet.markers} callout markers</Text>
							</View>
				...
				<View className="mt-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
					<View className="px-5 pt-4 pb-3">
						<Text className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Recent Photos</Text>
					</View>
					<View className="flex-row flex-wrap px-4 pb-4 gap-2">
						{["rebar42", "foundation55", "concrete63", "scaffold71", "formwork28", "steelbeam19"].map((seed, i) => (
							<View
								key={i}
								className="rounded-xl overflow-hidden"
								style={{ width: "31%", aspectRatio: 1 } as any}
							>
								<Image source={{ uri: `https://picsum.photos/seed/${seed}/300/300` }} style={{ width: "100%", height: "100%" }} resizeMode="cover" />
							</View>
						))}
					</View>
				</View>
...
function ShareProjectFlow({ initialPhase = "project-detail" as FlowPhase }: { initialPhase?: FlowPhase }) {
		...
		return (
			<>
				<ShareOptionsSheet
					...
					onCopy={() => { setPhase("link-copied"); }}
					onSend={(method) => { setToastMsg(`Opening ${method}...`); setTimeout(() => setPhase("link-copied"), 1200); }}
				/>
				<StoryToast message={toastMsg} visible={!!toastMsg} onDismiss={() => setToastMsg("")} />
			</>
		);
...
export const ProjectDetail: Story = { name: "1. Project Detail", args: { initialPhase: "project-detail" } };
export const ShareOptions: Story = { name: "2. Share Options", args: { initialPhase: "share-options" } };
export const LinkCopied: Story = { name: "3. Link Copied", args: { initialPhase: "link-copied" } };
export const RecipientView: Story = { name: "4. Recipient View", args: { initialPhase: "recipient-view" } };
