...
import { CreateProjectOverlay, FlatRow, SectionGroup, StoryToast } from "@/app/_story-components"
  ...
  return (
    ...
    >
      <View className="items-center gap-6">
        <Icon as={Zap} style={{ color: "#3b82f6" }} className="size-10" />
        ...
        <View className="w-full pt-4">
          {[
            ...
            <View key={item.title}>
              {i > 0 && (
                <View
                  style={{ height: 1, backgroundColor: "rgba(255,255,255,0.06)", marginLeft: 40 }}
                />
              )}
...
function TrialStartScreen({ onContinue, onSkip }: { onContinue: () => void; onSkip: () => void }) {
  return (
    <ScrollView className="bg-background flex-1" contentContainerClassName="px-6 py-12">
      ...
      <SectionGroup title="What's included">
        {[
          "5 projects",
          "15 team members",
          "Unlimited sheets",
          "Auto callout detection (96% accuracy)",
          ...
          <View key={label}>
            {i > 0 && (
              <View
                style={{ height: 1, backgroundColor: "rgba(255,255,255,0.06)", marginLeft: 32 }}
              />
            )}
            <View className="flex-row items-center gap-3 px-4 py-3">
  ...
  return (
    <View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
      ...
      <View className="flex-1 items-center justify-center px-8">
        <Icon as={FolderOpen} className="text-muted-foreground mb-6 size-10" />
        ...
        <Text className="text-muted-foreground mb-8 text-center text-sm leading-relaxed">
          Create your first project to start managing construction plans with AI-powered
          intelligence.
        </Text>
...
const PROCESSING_STEPS = [
  {
    key: "upload",
    label: "Uploading plan",
    icon: Upload,
    color: "#3b82f6",
    detail: "structural-plans-v3.pdf",
  },
  {
    key: "pages",
    ...
    icon: FileText,
    color: "#3b82f6",
    detail: "12 sheets detected",
  },
  {
    key: "metadata",
    label: "Reading sheet titles",
    icon: Layers,
    color: "#a855f7",
    detail: "S1.0, S2.0, S3.0...",
  },
  {
    key: "callouts",
    ...
    icon: ScanLine,
    color: "#f59e0b",
    detail: "Scanning for detail markers, sections, elevations...",
  },
  {
    key: "linking",
    label: "Linking references",
    icon: Link2,
    color: "#22c55e",
    detail: "Matching callouts to destination sheets",
  },
  {
    key: "tiles",
    label: "Generating tiles",
    icon: Zap,
    color: "#22c55e",
    detail: "Creating zoomable view layers",
  },
]
  ...
  return (
    <View className="bg-background" style={{ minHeight: "100vh" } as any}>
      ...
      <View className="flex-1 px-6">
        ...
        >
          <Icon as={Upload} className="text-primary mb-5 size-8" />
        ...
        <SectionGroup title="What happens next">
          {[
            { icon: ScanLine, color: "#f59e0b", text: "AI scans every sheet for callout symbols" },
            {
              icon: Link2,
              color: "#22c55e",
              text: "Callouts are automatically linked to their target details",
            },
            {
              icon: Sparkles,
              color: "#a855f7",
              text: "Schedules and tables are extracted for quick reference",
            },
          ].map((item, i) => (
            <View key={i}>
              {i > 0 && (
                <View
                  style={{ height: 1, backgroundColor: "rgba(255,255,255,0.06)", marginLeft: 36 }}
                />
              )}
              <FlatRow icon={item.icon} iconColor={item.color} label={item.text} />
            </View>
          ))}
        </SectionGroup>
...
function ProcessingScreen({ onComplete }: { onComplete: () => void }) {
  ...
  const progress = isComplete
    ? 100
    : ((stepIndex + Math.min(subProgress, 100) / 100) / PROCESSING_STEPS.length) * 100
          ...
          <>
            <View
              ...
              style={{
                width: 80,
                height: 80,
                backgroundColor: (currentStep?.color ?? "#3b82f6") + "15",
              }}
            >
              <Icon
  ...
  return (
    <View className="bg-background" style={{ minHeight: "100vh" } as any}>
      <View className="px-6 pt-8 pb-6">
        <Icon as={Sparkles} style={{ color: "#22c55e" }} className="mb-4 size-8 self-center" />
        <Text className="text-foreground text-center text-2xl font-bold">Your Plans Are Ready</Text>
        <Text className="text-muted-foreground mt-1 text-center text-sm">
        ...
        <View className="mt-6 flex-row gap-4">
          {stats.map((stat) => (
            <View key={stat.label} className="flex-1 items-center py-4">
        ...
        <View className="mt-4 w-full">
          ...
          <View style={{ height: 1, backgroundColor: "rgba(255,255,255,0.06)", marginLeft: 36 }} />
          <View className="flex-row items-center gap-2 px-4 py-3">
      ...
      <View className="flex-1">
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator>
          {MOCK_SHEETS.map((sheet, i) => (
            <View key={sheet.id}>
              {i > 0 && (
                <View
                  style={{ height: 1, backgroundColor: "rgba(255,255,255,0.06)", marginLeft: 16 }}
                />
              )}
              <Pressable className="active:bg-muted/20 flex-row items-center gap-3 px-6 py-3">
                <View className="flex-1">
                  <View className="flex-row items-center gap-2">
...
function OnboardingFlow({ initialStep = "splash" as FlowStep }: { initialStep?: FlowStep }) {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case "trial":
        return (
          <TrialStartScreen onContinue={() => setStep("signup")} onSkip={() => setStep("signup")} />
        )
      case "signup":
