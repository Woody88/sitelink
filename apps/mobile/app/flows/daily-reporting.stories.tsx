  ...
  return (
    <View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
      ...
      <ScrollView className="flex-1" contentContainerClassName="px-4 pb-8">
          ...
          <View className="gap-3 px-5 py-4">
            ...
            <View className="gap-2">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-foreground text-sm">Holabird Ave Warehouse</Text>
              </View>
              <View className="flex-row items-center gap-2">
                <Icon as={Calendar} className="text-muted-foreground size-3.5" />
                <Text className="text-foreground text-sm">March 9, 2026</Text>
              </View>
              <View className="flex-row items-center gap-2">
                <Icon as={Cloud} className="text-muted-foreground size-3.5" />
                <Text className="text-foreground text-sm">Clear, 45°F</Text>
              </View>
            </View>
            <View className="mt-1 flex-row gap-4">
              <View className="flex-row items-center gap-1.5">
                <Icon as={Camera} className="text-muted-foreground size-3.5" />
                <Text className="text-muted-foreground text-xs">5 photos</Text>
              </View>
              <View className="flex-row items-center gap-1.5">
                <Icon as={Mic} className="text-muted-foreground size-3.5" />
                <Text className="text-muted-foreground text-xs">1 voice note</Text>
              </View>
              <View className="flex-row items-center gap-1.5">
                <Icon as={AlertTriangle} className="size-3.5" style={{ color: "#ef4444" }} />
        ...
        <View className="mb-6" style={{ height: 1, backgroundColor: "rgba(255,255,255,0.06)" }} />
        ...
        <View className="mb-6">
          ...
          <View className="flex-row flex-wrap gap-2">
            {REPORT_TEXT.photos.map((photo) => (
              ...
              >
                <Image
                  source={{ uri: `https://picsum.photos/seed/${photo.seed}/200/200` }}
      ...
      >
        <Pressable className="items-center gap-1" onPress={onRegenerate}>
          <Icon as={RefreshCcw} className="text-foreground size-5" />
          <Text className="text-foreground text-[10px]">Regenerate</Text>
        </Pressable>
        <Pressable className="items-center gap-1" onPress={onEdit}>
          <Icon as={Edit2} className="text-foreground size-5" />
          <Text className="text-foreground text-[10px]">Edit</Text>
        </Pressable>
    ...
    <View className="bg-background flex-1" style={{ minHeight: "100vh" } as any}>
      ...
      <View className="flex-1 px-4 pt-2 pb-4">
        <View className="mb-3 flex-row items-center gap-2">
          ...
          <Text className="text-muted-foreground text-xs">
            Edit the AI-generated text before sharing
          </Text>
        </View>
        <View
        ...
        <View className="flex-row items-center justify-between px-6 pb-2">
          <View>
            ...
            <Text className="text-muted-foreground text-sm">
              Daily Construction Report — March 9
            </Text>
          </View>
      ...
      <View className="items-center gap-5 px-8">
        ...
        >
          <Text className="text-muted-foreground text-center text-xs">
            Daily Construction Report — March 9, 2026
          </Text>
        </View>
...
function DailyReportingFlow({
  initialPhase = "prompt" as FlowPhase,
}: {
  ...
  if (phase === "edit")
    return <ReportEditView onBack={() => setPhase("report")} onShare={() => setPhase("share")} />
  if (phase === "share")
    return (
      <ReportShareSheet
  ...
  if (phase === "share-confirm")
    return <ShareConfirmationView action={shareAction} onDone={() => setPhase("report")} />
  if (phase === "report")
    return (
      <ReportView
        onRegenerate={() => {
      ...
      <View className="items-center gap-4">
        ...
        <View className="mt-2 flex-row items-center gap-4">
          <View className="flex-row items-center gap-1.5">
            ...
            <Text className="text-muted-foreground text-sm">5 photos</Text>
          </View>
          <View className="flex-row items-center gap-1.5">
            <Icon as={Mic} className="text-muted-foreground size-4" />
            <Text className="text-muted-foreground text-sm">1 voice note</Text>
          </View>
        </View>
...
export const ShareConfirmation: Story = {
  name: "6. Share Confirmation",
  args: { initialPhase: "share-confirm" },
}
export const GenerationError: Story = {
  name: "7. AI Error — Retry",
  args: { initialPhase: "error" },
}
