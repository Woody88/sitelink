				...
				<View className="gap-3">
					{[1, 2, 3, 4, 5].map((i) => (
						...
						>
							...
							<Icon as={ChevronRight} className="size-4" style={{ color: "rgba(255,255,255,0.1)" }} />
						</View>
					))}
				</View>
			</ScrollView>
					...
					<View key={cluster} className="mb-2">
						...
						<View className="gap-2 px-4">
							{[1, 2, 3].slice(0, cluster === 3 ? 2 : 3).map((photo) => (
								...
								>
									<Skeleton className="h-[90px] w-[120px]" style={{ borderTopLeftRadius: 16, borderBottomLeftRadius: 16 } as any} />
									<View className="flex-1 justify-center gap-2 px-3 py-2">
										<Skeleton className="h-3 w-16 rounded" />
										<Skeleton className="h-3 w-10 rounded" />
									</View>
									<View className="items-center justify-center pr-3">
										<Icon as={ChevronRight} className="size-4" style={{ color: "rgba(255,255,255,0.08)" }} />
									</View>
								</View>
							))}
						</View>
...
function LoadingStatesComponent({ variant = "sheets" }: { variant?: "sheets" | "projects" | "photos" }) {
...
export const ProjectList: Story = { name: "2. Project List Skeleton", args: { variant: "projects" } };
export const PhotoTimeline: Story = { name: "3. Photo Timeline Skeleton", args: { variant: "photos" } };
