...
const NOTIFICATIONS = [
  ...
  },
  {
    ...
    body: "Sarah Johnson accepted your invitation.",
    time: "4 days ago",
    type: "success" as const,
  },
  {
    ...
    body: "Thompson Structural responded to RFI-2026-0045.",
    time: "5 days ago",
    type: "info" as const,
  },
]
...
function PlanDestinationScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <View
      style={
        { flex: 1, position: "relative", backgroundColor: "#0a0a0a", minHeight: "100vh" } as any
      }
    >
      <Image
...
function NotificationsFlow({
  initialScreen = "list" as FlowScreen,
}: {
        ...
        <View className="flex-1 px-4 pt-6">
          <View className="gap-3">
            <View className="flex-row items-center gap-2">
              <Icon
                as={ICON_MAP[selected.type]}
                className={cn("size-5", COLOR_MAP[selected.type])}
              />
              <Text className="text-foreground text-xl font-bold">{selected.title}</Text>
            </View>
            <Text className="text-muted-foreground text-sm leading-relaxed">{selected.body}</Text>
        ...
        {NOTIFICATIONS.map((item) => {
          ...
          return (
            <Pressable
              ...
              className="active:bg-muted/30 flex-row items-start gap-3 px-4 py-4"
            >
              {!isRead && (
                <View
              ...
              <Icon
                as={ICON_MAP[item.type]}
                className={cn("mt-0.5 size-5", COLOR_MAP[item.type])}
              />
...
export const NotificationList: Story = {
  name: "1. Notification List",
  args: { initialScreen: "list" },
}
export const NotificationDetail: Story = {
  name: "2. Notification Detail",
  args: { initialScreen: "detail" },
}
export const DeepLinkToPlan: Story = {
  name: "3. Deep Link to Plan",
  args: { initialScreen: "navigating-plan" },
}
export const DeepLinkToPhotos: Story = {
  name: "4. Deep Link to Photos",
  args: { initialScreen: "navigating-photos" },
}
