...
const TYPE_CONFIG: Record<
  ResultType,
  { icon: typeof FileText; label: string; color: string; bg: string; iconClass: string }
> = {
  sheet: {
    icon: FileText,
    label: "Sheet",
...
function HighlightedSnippet({ text, term }: { text: string; term: string }) {
  ...
  return (
    <Text className="text-muted-foreground text-sm" numberOfLines={2}>
      ...{text.slice(0, idx)}
      <Text className="text-foreground font-bold">{text.slice(idx, idx + term.length)}</Text>
      ...
      >
        <View className="rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(28,28,28,0.95)" }}>
          <Text className="text-muted-foreground text-xs font-semibold uppercase tracking-wide">
            Search match on {result.sheetNumber}
          </Text>
        ...
        {showEmpty && (
          <View className="px-4 pt-6">
            ...
            <View className="mt-8 items-center">
              <Icon as={Search} className="text-muted-foreground mb-4 size-8" />
...
export const WithResults: Story = {
  name: "2. Results for 'concrete'",
  args: { initialQuery: "concrete" },
}
