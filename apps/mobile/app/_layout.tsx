...
export default function RootLayout() {
  if (isPrototypeMode()) {
    return <PrototypeRootLayout />
  }
  return <AuthRootLayout />
}

function PrototypeRootLayout() {
  const { theme } = useUniwind()
  const navTheme = NAV_THEME[theme === "dark" ? "dark" : "light"]
  const statusBarStyle = theme === "dark" ? "light" : "dark"
  const protoSession = getPrototypeSessionContext()
  ...
  return (
    <SessionProvider sessionData={protoSession} isPending={false} refetch={async () => {}}>
      <PrototypeProvider>
    ...
    return (
      <ThemeProvider value={navTheme}>
        <View className="bg-background flex-1 items-center justify-center">
          <Text className="text-muted-foreground">Setting up your workspace...</Text>
        </View>
        <StatusBar style={statusBarStyle} />
        <PortalHost />
      </ThemeProvider>
  ...
  return (
    <LiveStoreErrorBoundary>
      <Suspense fallback={<Text>Loading LiveStore...</Text>}>
        <StoreRegistryProvider key={sessionKey} storeRegistry={getStoreRegistry()}>
          <ThemeProvider value={navTheme}>
            <ProjectProvider>
              <Stack screenOptions={{ headerShown: false }} />
            </ProjectProvider>
