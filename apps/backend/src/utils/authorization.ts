...
import type { Env, AuthorizationResult } from '../types/env';
...
export async function getUserAccessibleProjects(
  ...
  const orgIds = orgs.results.map((o: { organizationId: string }) => o.organizationId);
...
export async function validateEventAuthorization(
  ...
  if (event.type === 'v1.ProjectCreated') {
    const isAdmin = await checkOrganizationAdmin(event.organizationId, userId, env);
    if (!isAdmin) {
      return {
        allowed: false,
  ...
  if (event.type === 'v1.ProjectUpdated' || event.type === 'v1.ProjectDeleted') {
    const result = await checkProjectEditPermission(event.projectId, userId, env);
    if (!result.allowed) {
      return result;
    }
  }
  ...
  if (
    event.type === 'v1.PhotoCaptured' ||
    event.type === 'v1.MarkerCreated'
  ) {
