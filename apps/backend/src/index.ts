...
export default {
	async fetch(
		...
		if (
			url.pathname === "/api/test/trigger-pipeline" &&
			request.method === "POST"
		) {
			try {
		...
		if (
			url.pathname.startsWith("/api/test/coordinator/") &&
			request.method === "GET"
		) {
		...
		if (url.pathname === "/api/test/upload-pdf" && request.method === "POST") {
			try {
				...
				const file = formData.get("file") as File;
				const projectId =
					(formData.get("projectId") as string) || "test-project";
				const organizationId =
					(formData.get("organizationId") as string) || "test-org";
		...
		if (url.pathname === "/api/events" && request.method === "GET") {
			try {
				const organizationId =
					url.searchParams.get("organizationId") || "test-org";
