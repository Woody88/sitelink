...
import { uploadPdfAndTriggerPipeline, parseR2EventPath } from "../processing/r2-with-notifications"
...
describe("R2 Upload Functionality", () => {
  ...
  it("should upload PDF to R2 bucket with correct metadata", async () => {
    ...
    await uploadPdfAndTriggerPipeline(mockEnv as Env, pdfPath, pdfData.buffer, {
      planId: testPlanId,
    ...
    expect(uploadedFile?.metadata?.httpMetadata?.contentType).toBe("application/pdf")
    expect(uploadedFile?.metadata?.customMetadata?.planId).toBe(testPlanId)
    expect(uploadedFile?.metadata?.customMetadata?.projectId).toBe(testProjectId)
    expect(uploadedFile?.metadata?.customMetadata?.organizationId).toBe(testOrganizationId)
  })
