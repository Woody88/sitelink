	...
	for (const message of batch.messages) {
		...
		try {
			...
			for (let i = 0; i < result.sheets.length; i++) {
				...
				console.log(
					`[ImageGeneration] Uploaded ${r2Path} (${pngData.byteLength} bytes)`,
				);
				...
				try {
					...
					await liveStoreStub.fetch(
						"http://internal/commit?storeId=" + job.organizationId,
						{
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({
	...
	for (const message of batch.messages) {
		...
		try {
			...
			if (result.isValid && !sheetNumber) {
				console.warn(
					`[MetadataExtraction] Sheet ${job.sheetId} is valid but has NO sheet number!`,
				);
			}
			await coordinator.sheetMetadataExtracted(
				job.sheetId,
				result.isValid,
				sheetNumber,
			);
			...
			console.error(
				`[MetadataExtraction] Failed for sheet ${job.sheetId}:`,
				error,
			);
			message.retry();
		}
	...
	for (const message of batch.messages) {
		...
		try {
			...
			try {
				...
				console.warn(
					`[CalloutDetection] LiveStore emit failed for ${job.sheetId}:`,
					liveStoreError,
				);
			}
			...
			console.error(
				`[CalloutDetection] Failed for sheet ${job.sheetId}:`,
				error,
			);
			message.retry();
		}
	...
	for (const message of batch.messages) {
		...
		try {
			...
			try {
				...
				console.warn(
					`[TileGeneration] LiveStore emit failed for ${job.sheetId}:`,
					liveStoreError,
				);
			}
