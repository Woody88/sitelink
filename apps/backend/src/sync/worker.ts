...
import { makeWorker, makeDurableObject } from "@livestore/sync-cf/cf-worker"
...
export class WebSocketServer extends makeDurableObject({
  onPush: async (message, context) => {
    ...
    console.log("[Sync] onPush: Received batch with", message.batch.length, "events")
  ...
  onPull: async (message, context) => {
    ...
    console.log("[Sync] onPull: Client requesting events since", message.cursor)
...
async function validatePayload(
  ...
  if (!sessionResult) {
    console.error("[Sync] Invalid or expired session for token:", authToken.substring(0, 10) + "...")
