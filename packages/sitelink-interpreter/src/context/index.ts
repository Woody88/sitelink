...
function getDefaultContext(legendSheetId: string | null): ProjectContext {
  return {
    ...
    symbols: [
      ...
      {
        shape: "circle_with_triangle_top",
        meaning: "elevation_callout",
        pattern: "([A-Z]|\\d+)",
        example: "2",
      },
      {
        shape: "circle_with_triangles_sides",
        meaning: "section_cut",
        pattern: "([A-Z])",
        example: "A",
      },
      {
        shape: "circle_with_line_below",
        meaning: "title_callout",
        pattern: "(\\d+|[A-Z])/([A-Z]\\d+)",
        example: "4/A7",
      },
    ],
...
async function detectLegendSheet(sheetsList: Sheet[]): Promise<Sheet | null> {
  for (const sheet of sheetsList) {
    if (
      sheet.sheet_number?.toLowerCase().includes("g") ||
