...
const CLASS_LABELS = ["detail_callout", "elevation_callout", "title_callout", "not_a_callout"]
  ...
  function drawCalloutOnCanvas(entity: Entity) {
    ...
    img.onload = () => {
      ...
      const viewX = Math.max(
        0,
        Math.min(img.width - viewWidth, croppedViewX * (1 - t) + fullViewX * t),
      )
      const viewY = Math.max(
        0,
        Math.min(img.height - viewHeight, croppedViewY * (1 - t) + fullViewY * t),
      )

      canvas.width = canvasWidth
      canvas.height = canvasHeight

      ctx.drawImage(img, viewX, viewY, viewWidth, viewHeight, 0, 0, canvasWidth, canvasHeight)
      ...
      ctx.beginPath()
      ctx.arc(
        highlightX + highlightW / 2,
        highlightY + highlightH / 2,
        Math.max(highlightW, highlightH) * 1.5,
        0,
        Math.PI * 2,
      )
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <header className="max-w-6xl mx-auto mb-6 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              setCurrentIndex(Math.min(entities.length - 1, currentIndex + 1))
              setZoomLevel(0)
            }}
        ...
        <div className="flex flex-wrap items-center gap-4 justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-gray-400">
              Sheet {entity.sheet_number} | Confidence:{" "}
          ...
          <div className="flex items-center gap-3">
            ...
            {CLASS_LABELS.filter((l) => l !== entity.class_label && l !== "not_a_callout").map(
              (label) => (
                <button
                  key={label}
      ...
      <div className="max-w-6xl mx-auto bg-gray-800 rounded-lg p-4">
        ...
        <div className="mt-2 text-xs text-gray-500 text-center">
          Entity {entity.id} | Bbox: ({entity.bbox_x1}, {entity.bbox_y1}) - ({entity.bbox_x2},{" "}
          {entity.bbox_y2})
        </div>
      </div>
    </div>
