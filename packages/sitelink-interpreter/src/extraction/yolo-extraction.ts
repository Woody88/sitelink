...
const YOLO_MODEL = join(
  import.meta.dir,
  "../../../callout-processor-v6-experimental/weights/best.pt",
)
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= numPages; pageNum++) {
      ...
      if (existsSync(detectionsPath)) {
        ...
        const maxX = Math.max(
          ...pageDetections.detections.map((d) => (d.bbox[0] ?? 0) + (d.bbox[2] ?? 0)),
          0,
        )
        const maxY = Math.max(
          ...pageDetections.detections.map((d) => (d.bbox[1] ?? 0) + (d.bbox[3] ?? 0)),
          0,
        )
    ...
    console.log(
      `v6 iteration 5 detection complete: ${summary.total_detections} detections across ${summary.total_sheets} sheets`,
    )
    return summary
  } catch (error) {
...
export async function extractWithYOLO(
  ...
  const { confThreshold = 0.25, standard = "auto", validate = false, useGemini = false } = options
  ...
  console.log(
    `v6 iteration 5 detection complete: ${totalDetections} detections across ${allSheets.length} sheets`,
  )
