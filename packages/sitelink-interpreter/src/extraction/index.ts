...
export function applyCorrection(
  classLabel: string,
  confidence: number,
): { label: string; wasCorrect: boolean } {
  const rule = correctionRules.find((r) => r.original_label === classLabel)

  if (rule && rule.count >= 2) {
    console.log(
      `    Applying correction: ${classLabel} → ${rule.corrected_label} (learned from ${rule.count} reviews)`,
    )
...
function parseCalloutLabel(label: string): {
...
export async function extractFromSheet(
  ...
  for (const marker of result.markers) {
    ...
    const entity = entities.insert({
      ...
      ocr_confidence: null,
      detection_method: "legacy_v3",
      standard: context?.standard ?? null,
      raw_ocr_text: marker.label,
...
export async function extractAll(
  ...
  const needsReview = results.reduce(
    (sum, r) => sum + r.entities.filter((e) => e.needs_review).length,
    0,
  )
