...
import {
  ...
  detectOnSheet,
  getCropPath,
  type YOLOExtractionOptions,
} from "../extraction/yolo-extraction.ts"
  ...
  routes: {
    ...
    "/api/entities/:id/provenance": (req) => {
      ...
      return Response.json({
        ...
        references:
          provenance?.references.map((e) => ({
            ...e,
            sheet_number: getSheetForEntity(e)?.sheet_number,
          })) ?? [],
        referenced_by:
          provenance?.referencedBy.map((e) => ({
            ...e,
            sheet_number: getSheetForEntity(e)?.sheet_number,
          })) ?? [],
      })
      ...
      async POST(req) {
        ...
        db.run(
          ...
          [
            ...
            entity.id,
            entity.class_label,
            body.corrected_label ?? entity.class_label,
            "hitl-ui",
          ],
        )
    ...
    "/api/retrain": {
      async POST() {
        try {
          ...
          const trainingCount = db.query(`SELECT COUNT(*) as count FROM training_data`).get() as {
            count: number
          }
          if (trainingCount.count === 0) {
            return Response.json(
              { error: "No training data. Review some entities first." },
              { status: 400 },
            )
          }
          ...
          const beforeCount = db.query(`SELECT COUNT(*) as count FROM entities`).get() as {
            count: number
          }
    ...
    "/api/metrics": () => {
      ...
      const misclassified = trainingData
        .filter(
          (row) =>
            row.original_label !== row.corrected_label && row.corrected_label !== "not_a_callout",
        )
        .reduce((sum, row) => sum + row.count, 0)
