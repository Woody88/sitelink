...
export const sheets = {
  ...
  getByPdf(pdfPath: string): Sheet[] {
    return getDb()
      .query<Sheet, [string]>("SELECT * FROM sheets WHERE pdf_path = ? ORDER BY page_number")
      .all(pdfPath)
  },

  getBySheetNumber(sheetNumber: string): Sheet | null {
    return getDb()
      .query<Sheet, [string]>("SELECT * FROM sheets WHERE sheet_number = ?")
      .get(sheetNumber)
  },
...
export const entities = {
  ...
  getNeedingReview(): Entity[] {
    return getDb()
      .query<Entity, []>("SELECT * FROM entities WHERE needs_review = 1 AND reviewed = 0")
      .all()
  },
...
export const relationships = {
  ...
  getBySource(entityId: string): Relationship[] {
    return getDb()
      .query<Relationship, [string]>("SELECT * FROM relationships WHERE source_entity_id = ?")
      .all(entityId)
  },

  getByTarget(entityId: string): Relationship[] {
    return getDb()
      .query<Relationship, [string]>("SELECT * FROM relationships WHERE target_entity_id = ?")
      .all(entityId)
  },
}
...
export const detectionRuns = {
  ...
  getLatest(): DetectionRun | null {
    return getDb()
      .query<DetectionRun, []>("SELECT * FROM detection_runs ORDER BY created_at DESC LIMIT 1")
      .get()
  },

  getByPdf(pdfPath: string): DetectionRun[] {
    return getDb()
      .query<DetectionRun, [string]>(
        "SELECT * FROM detection_runs WHERE pdf_path = ? ORDER BY created_at DESC",
      )
      .all(pdfPath)
  },
}
