...
export function getProvenanceChain(
  entityId: string,
): { entity: Entity; references: Entity[]; referencedBy: Entity[] } | null {
...
if (import.meta.main) {
  ...
  if (sampleEntity) {
    ...
    if (provenance) {
      ...
      console.log(
        `  References: ${provenance.references.map((e) => e.ocr_text).join(", ") || "none"}`,
      )
      console.log(
        `  Referenced by: ${provenance.referencedBy.map((e) => e.ocr_text).join(", ") || "none"}`,
      )
    }
  }
}
