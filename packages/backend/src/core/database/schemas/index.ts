...
export const processingJobs = D.sqliteTable("processing_jobs", {
	...
	uploadId: D.text("upload_id")
		.notNull()
		.references(() => files.uploadId, { onDelete: "cascade" }),
	planId: D.text("plan_id")
		.notNull()
	...
	pdfPath: D.text("pdf_path").notNull(),
	status: D.text()
		.notNull()
		.default("pending"), // "pending", "processing", "complete", "partial_failure", "failed"
	totalPages: D.integer("total_pages"),
	completedPages: D.integer("completed_pages").default(0),
	failedPages: D.text("failed_pages"), // JSON array of failed page numbers
