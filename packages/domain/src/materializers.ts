...
export const materializers = State.SQLite.materializers(events, {
  ...
  "v1.PlanMetadataCompleted": (event) => [
    // Delete sheets that are NOT in the validSheets array
    // LiveStore 0.4 syntax: { op: 'NOT IN', value: array }
    tables.sheets.delete().where({
      planId: event.planId,
      id: { op: "NOT IN", value: event.validSheets },
    }),
  ],
  ...
  // ===================
  // Layout region materializers
  // ===================
  "v1.SheetLayoutRegionsDetected": (event) =>
    event.regions.map((region) =>
      tables.layoutRegions.insert({
        id: region.id,
        sheetId: event.sheetId,
  ...
  // ===================
  // Schedule extraction materializers
  // ===================
  "v1.SheetScheduleExtracted": (event) =>
    event.entries.map((entry) =>
      tables.scheduleEntries.insert({
        id: entry.id,
  ...
  // ===================
  // Notes extraction materializers
  // ===================
  "v1.SheetNotesExtracted": (event) =>
  ...
  // ===================
  // Legend crop materializers
  // ===================
  "v1.SheetLegendCropped": (event) =>
    tables.layoutRegions
      .update({
  ...
  // ===================
  // Grid bubble materializers
  // ===================
  "v1.SheetGridBubblesDetected": (event) =>
    event.bubbles.map((bubble) =>
      tables.gridBubbles.insert({
        id: bubble.id,
        sheetId: event.sheetId,
        label: bubble.label,
        axis: null, // Phase 2
