...
export const events = {
  ...
  planMetadataCompleted: Events.synced({
    ...
    schema: Schema.Struct({
      ...
      sheetNumberMap: Schema.Record({
        key: Schema.String,
        value: Schema.String,
      }), // Map of sheetId -> sheetNumber (e.g., "sheet-0" -> "A1")
  ...
  // Marker events
  // ===================
  // Note: All marker x/y coordinates are normalized (0-1 range relative to image dimensions)
    ...
    schema: Schema.Struct({
      ...
      markers: Schema.Array(
        Schema.Struct({
          ...
          label: Schema.String,
          targetSheetId: Schema.optional(Schema.String),
          x: Schema.Number, // Normalized 0-1 coordinate
          y: Schema.Number, // Normalized 0-1 coordinate
          confidence: Schema.Number,
        }),
      ),
    }),
  ...
  markerUpdated: Events.synced({
    ...
    schema: Schema.Struct({
      ...
      x: Schema.optional(Schema.Number), // Normalized 0-1 coordinate
      y: Schema.optional(Schema.Number), // Normalized 0-1 coordinate
      targetSheetId: Schema.optional(Schema.String),
    }),
  }),
  ...
  // ===================
  // Layout region events (DocLayout-YOLO detection)
  // ===================
  sheetLayoutRegionsDetected: Events.synced({
    name: "v1.SheetLayoutRegionsDetected",
    schema: Schema.Struct({
      sheetId: Schema.String,
      regions: Schema.Array(
        Schema.Struct({
          id: Schema.String,
          regionClass: Schema.String, // "schedule" | "notes" | "legend"
          regionTitle: Schema.optional(Schema.String),
          x: Schema.Number,
          y: Schema.Number,
          width: Schema.Number,
          height: Schema.Number,
  ...
  // ===================
  // Schedule extraction events (LLM on cropped schedule regions)
  // ===================
  sheetScheduleExtracted: Events.synced({
    name: "v1.SheetScheduleExtracted",
    schema: Schema.Struct({
      sheetId: Schema.String,
      regionId: Schema.String,
      scheduleType: Schema.String, // "footing" | "pier" | "column" | "beam"
      entries: Schema.Array(
        Schema.Struct({
          id: Schema.String,
          mark: Schema.String, // "F1" | "P1" | "C2"
          properties: Schema.String, // JSON string: {size, reinforcing, notes, ...}
          confidence: Schema.Number,
          createdAt: Schema.Number,
        }),
      ),
  ...
  // ===================
  // Notes extraction events (LLM on cropped notes regions)
  // ===================
  sheetNotesExtracted: Events.synced({
    name: "v1.SheetNotesExtracted",
    schema: Schema.Struct({
      sheetId: Schema.String,
      regionId: Schema.String,
      content: Schema.String,
      noteType: Schema.String, // "general_notes" | "concrete_notes" | etc.
      extractedAt: Schema.Number,
    }),
  }),

  // ===================
  // Legend crop events (image crop only, no LLM extraction)
  // ===================
  sheetLegendCropped: Events.synced({
    name: "v1.SheetLegendCropped",
    schema: Schema.Struct({
      sheetId: Schema.String,
      regionId: Schema.String,
      cropImageUrl: Schema.String,
      croppedAt: Schema.Number,
    }),
  }),

  // ===================
  // Grid bubble detection events (YOLO 4-class model)
  // ===================
  sheetGridBubblesDetected: Events.synced({
    name: "v1.SheetGridBubblesDetected",
    schema: Schema.Struct({
      sheetId: Schema.String,
      bubbles: Schema.Array(
        Schema.Struct({
          id: Schema.String,
          label: Schema.String, // "A" | "B" | "1" | "2"
          x: Schema.Number,
          y: Schema.Number,
          width: Schema.Number,
          height: Schema.Number,
